...
open class MyBanner: UIView {
    ...
    @objc open var minimumHeight: CGFloat = 90
    ...
    @objc public let detailLabel: UILabel = {
        let label = UILabel()
        //label.font = UIFont.primaryEnglish(size: 12)
    ...
    private func addGestureRecognizers() {
        addGestureRecognizer(UITapGestureRecognizer(target: self, action:  #selector(MyBanner.didTap(_:))))
        let swipe = UISwipeGestureRecognizer(target: self, action: #selector(MyBanner.didSwipe(_:)))
        swipe.direction = .up
    ...
    private func adjustHeightOffset() {
        ...
        if superview === MyBanner.topWindow() && self.position == .top {
    ...
    open func show(_ view: UIView? = nil, duration: TimeInterval? = nil) {
        let viewToUse = view ?? MyBanner.topWindow()
        ...
        UIView.animate(withDuration: animationDuration, delay: 0.3, usingSpringWithDamping: damping, initialSpringVelocity: velocity, options: .allowUserInteraction, animations: {
